...
export async function signInAsGuest(customName = '') {
    const { data, error } = await supabase.auth.signInAnonymously();
    if (error) throw error;

    // Attach custom name to user object so upsertProfile will use it instead of 'ضيف'
    if (customName) {
        data.user.user_metadata = data.user.user_metadata || {};
        data.user.user_metadata.name = customName;
        // Generate a random avatar color and URL for the guest
        const bgColors = ['7c3aed', 'be185d', '3b82f6', '10b981', 'f59e0b', 'ec4899'];
        const randomBg = bgColors[Math.floor(Math.random() * bgColors.length)];
        data.user.user_metadata.avatar_url = `https://ui-avatars.com/api/?name=${encodeURIComponent(customName)}&background=${randomBg}&color=fff&rounded=true`;
    }
