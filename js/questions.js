...
// ── Simple predictable PRNG for seeding ──────────────────────
...
// ── Fetch active questions based on room config ──────────────
export async function getActiveQuestions(roomId, userId, roomConfig) {
    ...
    const qCount = roomConfig.question_count || 10;
    const sameOpts = roomConfig.same_questions !== undefined ? roomConfig.same_questions : true;

    // Create a seed based on room config
    const seed = sameOpts ? roomId : `${roomId}-${userId}`;
    const rng = seededRandom(seed);

    // Shuffle questions deterministically based on the seed
    ...
    return allQuestions.slice(0, Math.min(qCount, allQuestions.length));
}
...
function renderScale(container) {
    ...
    hint.textContent = '1 = أقل درجة  |  5 = أعلى درجة';
