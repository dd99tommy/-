...
// ── Generate a 6-character room code (2 letters, 4 numbers) ──
...
export async function createRoom(userId, config = {}) {
    ...
    const questionCount = config.question_count || 10;
    const sameQuestions = config.same_questions !== undefined ? config.same_questions : true;
...
export function getInviteLink(roomId) {
    return new URL('invite.html?room=' + roomId, window.location.href).href;
}
